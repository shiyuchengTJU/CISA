...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
                 random_start, targeted, class_, return_early, scale, beam_size=1):
        min_, max_ = a.bounds()
        s = max_ - min_
        ...
        for _ in range(iterations):
            ...
            for x_counter in range(len(saved_points)):
                for beam_counter in range(beam_size):
                    ...
                    temp_trajectory = trajectory(saved_points[x_counter].img, saved_points[x_counter].momentum)
                    gradient = self._gradient(a, temp_x, class_, strict=strict)
                    strict = True
                    if targeted:
                        gradient = -gradient

                    temp_trajectory.momentum += gradient
                    momentum_norm = np.sqrt(np.mean(np.square(temp_trajectory.momentum)))
                    momentum_norm = max(1e-8, momentum_norm)  # avoid divsion by zero
                    temp_trajectory.img = temp_trajectory.img + stepsize * (temp_trajectory.momentum/momentum_norm)
                    # temp_trajectory.img = temp_trajectory.img + stepsize * gradient


                    temp_trajectory.img = original + self._clip_perturbation(a, temp_trajectory.img - original, epsilon)
                    temp_trajectory.img = np.clip(temp_trajectory.img, min_, max_)

                    logits, is_adversarial = a.predictions(temp_trajectory.img)
                    temp_trajectory.score = softmax(logits)[class_] + (np.sum((temp_trajectory.img/255.0 - original/255.0) ** 2))**0.5

                    self.shit+=1
                    # print(self.shit)

                    temp_trajectory.is_adversarial = is_adversarial

                    new_points.append(temp_trajectory)
                    ...
                    if is_adversarial:
                        # print("logits", softmax(logits)[class_])
                        # print("dist", (np.sum((temp_trajectory.img/255.0 - original/255.0) ** 2))**0.5)
                        # print("query times", self.shit)
                        if return_early:
                            return True
                        else:
                            success = True
                ...
                # ori_new_trajectory.img = ori_new_trajectory.img + stepsize * gradient
                ...
                if is_adversarial:
                    # print("logits", softmax(logits)[class_])
                    # print("dist", (np.sum((ori_new_trajectory.img//255.0 - original/255.0) ** 2))**0.5)
                    # print("query times", self.shit)
                    if return_early:
                        return True
                    else:
            ...
            if len(new_points)<=(beam_size+1):
                ...
                # saved_points.append(new_points[-1])
