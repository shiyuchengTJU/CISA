...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run(self, a, binary_search,
             ...
             random_start, return_early, scale, norm, bb_step):
        ...
        if binary_search:
            ...
            return self._run_binary_search(
                ...
                random_start, targeted, class_, return_early, k=k, scale=scale, norm=norm, bb_step=bb_step)
        else:
            return self._run_one(
                a, epsilon, stepsize, iterations,
                random_start, targeted, class_, return_early, scale=scale, norm=norm, bb_step=bb_step)

    def _run_binary_search(self, a, epsilon, stepsize, iterations,
                           random_start, targeted, class_, return_early, k, scale, norm, bb_step):
        ...
        def try_epsilon(epsilon):
            ...
            return self._run_one(
                a, epsilon, stepsize, iterations,
                random_start, targeted, class_, return_early, scale, norm, bb_step)
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
                 random_start, targeted, class_, return_early, scale, beam_size=3, norm=True, bb_step=15):
        min_, max_ = a.bounds()
        s = max_ - min_
        ...
        # result_oriented
        x = x + self._clip_perturbation(a, 0.1 * stepsize * self.success_dir, epsilon)
        ...
        saved_points = []
        logtis_compare = []
        IFGSM_trajectory = trajectory(x)
        MIFGSM_trajectory = trajectory(x)
        vr_MIFGSM_trajectory = trajectory(x)

        
        #也就是三个都做
        for _ in range(iterations):
            #vr_MI_FGSM
            temp_x = np.clip(np.random.normal(loc=vr_MIFGSM_trajectory.img, scale=scale), min_, max_).astype(np.float32)
            ...
            vr_MIFGSM_trajectory.momentum += gradient
            momentum_norm = np.sqrt(np.mean(np.square(vr_MIFGSM_trajectory.momentum)))
            momentum_norm = max(1e-12, momentum_norm)  # avoid divsion by zero
            vr_MIFGSM_trajectory.img = vr_MIFGSM_trajectory.img + stepsize * (vr_MIFGSM_trajectory.momentum/momentum_norm)

            vr_MIFGSM_trajectory.img = original + self._clip_perturbation(a, vr_MIFGSM_trajectory.img - original, epsilon)
            vr_MIFGSM_trajectory.img = np.clip(vr_MIFGSM_trajectory.img, min_, max_)
            logits, is_adversarial = a.predictions(vr_MIFGSM_trajectory.img)
            ...
            if is_adversarial:
                ...
                #开始二分回溯
                left = original
                right = vr_MIFGSM_trajectory.img
            ...
            #MI_FGSM
            gradient = self._gradient(a, MIFGSM_trajectory.img, class_, strict=strict)
            strict = True
            if targeted:
                gradient = -gradient

            MIFGSM_trajectory.momentum += gradient
            momentum_norm = np.sqrt(np.mean(np.square(MIFGSM_trajectory.momentum)))
            momentum_norm = max(1e-12, momentum_norm)  # avoid divsion by zero

            MIFGSM_trajectory.img = MIFGSM_trajectory.img + stepsize * (MIFGSM_trajectory.momentum/momentum_norm)

            MIFGSM_trajectory.img = original + self._clip_perturbation(a, MIFGSM_trajectory.img - original, epsilon)
            MIFGSM_trajectory.img = np.clip(MIFGSM_trajectory.img, min_, max_)

            logits, is_adversarial = a.predictions(MIFGSM_trajectory.img)
            ...
            #I_FGSM
            gradient = self._gradient(a, IFGSM_trajectory.img, class_, strict=strict)
            strict = True
            if targeted:
                gradient = -gradient
            ...
            if norm:
                ...
                IFGSM_trajectory.img = IFGSM_trajectory.img + stepsize * (gradient/gradient_norm)
            else:
                IFGSM_trajectory.img = IFGSM_trajectory.img + stepsize * gradient

            IFGSM_trajectory.img = original + self._clip_perturbation(a, IFGSM_trajectory.img - original, epsilon)
            IFGSM_trajectory.img = np.clip(IFGSM_trajectory.img, min_, max_)

            logits, is_adversarial = a.predictions(IFGSM_trajectory.img)
