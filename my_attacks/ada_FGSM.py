...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
        ...
        for _ in range(iterations):
            ...
            alpha_est = (epsilon - alpha_bar * _) / (iterations - _)
            if _ == 0:  #第一次
                alpha_now = 0.9*epsilon/iterations
            else:
                temp_sig = sigmoid(gradient/(grad_bar+1e-6)) * 2
                alpha_now = (epsilon - alpha_total) * ((temp_sig) / (1e-6 + temp_sig + iterations- _ -1))

            alpha_total += alpha_now
            alpha_bar = alpha_total / (1+_) 
            grad_total += gradient
            grad_bar = grad_total / (1+_)
