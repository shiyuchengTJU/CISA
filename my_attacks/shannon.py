...
class ShannonL2Attack(Attack):
    ...
    def __call__(self, input_or_adv, label=None, unpack=True,
        ...
        for binary_search_step in range(binary_search_steps):
            ...
            for iteration in range(max_iterations):
                x = att_original + att_perturbation

                logits, is_adv = a.predictions(np.clip(x, 0, 255))
                loss, dldx = self.loss_function(const, a, x, logits, att_original, min_, max_)
    ...
    @classmethod
    def loss_function(cls, const, a, x, logits, reconstructed_original, min_, max_):
        ...
        prob = softmax(logits)
        shannon_loss = np.sum(prob * np.log(prob))    #shannon entropy

        print(prob[0])

        s = max_ - min_
        squared_l2_distance = np.sum((x - reconstructed_original)**2) / s**2
        total_loss = squared_l2_distance + const * shannon_loss

        # calculate the gradient of shannon_loss w.r.t. x
        logits_diff_grad = np.zeros_like(logits)

        for temp_i in range(len(logits)):    #chain rule
            for temp_j in range(len(logits)):
                if temp_i == temp_j:
                    logits_diff_grad[temp_i]+=(prob[temp_j]**2 - prob[temp_j])*(1+np.log(prob[temp_j]))
                else:
                    logits_diff_grad[temp_i]+=(prob[temp_j]*prob[temp_i]*(1+np.log(prob[temp_j])))

        shannon_loss_grad = a.backward(logits_diff_grad, np.clip(x, 0, 255))

        # assert shannon_loss >= 0

        squared_l2_distance_grad = (2 / s**2) * (x - reconstructed_original)

        total_loss_grad = squared_l2_distance_grad - const * shannon_loss_grad

        print(const * shannon_loss, squared_l2_distance)
