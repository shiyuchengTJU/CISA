...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run(self, a, binary_search,
             ...
             random_start, return_early, scale, bb_step):
        ...
        if binary_search:
            ...
            return self._run_binary_search(
                ...
                random_start, targeted, class_, return_early, k=k, scale=scale, bb_step=bb_step)
        else:
            return self._run_one(
                a, epsilon, stepsize, iterations,
                random_start, targeted, class_, return_early, scale=scale, bb_step=bb_step)

    def _run_binary_search(self, a, epsilon, stepsize, iterations,
                           random_start, targeted, class_, return_early, k, scale, bb_step):
        ...
        def try_epsilon(epsilon):
            ...
            return self._run_one(
                a, epsilon, stepsize, iterations,
                random_start, targeted, class_, return_early, scale, bb_step)
    ...
    def update_blank_dir(self, new_adv):
        if len(self.blank_adv)==0: #头一回
            new_adv_norm = np.sqrt(np.mean(np.square(new_adv)))
            new_adv_norm = max(1e-12, new_adv_norm)
            self.blank_dir = -new_adv/new_adv_norm
            self.blank_adv.append(-new_adv)

        else:
            temp_beta = new_adv
            for blank_iterator in self.blank_adv:
                temp_beta -= blank_iterator*(new_adv*blank_iterator)/(blank_iterator*blank_iterator+1e-7)
            self.blank_adv.append(temp_beta)
            blank_adv_norm = np.sqrt(np.mean(np.square(temp_beta)))
            blank_adv_norm = max(1e-12, blank_adv_norm)
            self.blank_dir = temp_beta/blank_adv_norm

        


    def _run_one(self, a, epsilon, stepsize, iterations,
                 random_start, targeted, class_, return_early, scale, bb_step=15):
        min_, max_ = a.bounds()
        s = max_ - min_
        ...
        # result_oriented
        x = x + self._clip_perturbation(a, stepsize * self.blank_dir, epsilon)
        # print("oriented:", x - original)
        ...
        for _ in range(iterations):
            ...
            momentum_norm = np.sqrt(np.mean(np.square(momentum)))
            momentum_norm = max(1e-12, momentum_norm)  # avoid divsion by zero
            x = x + stepsize * (momentum/momentum_norm)
