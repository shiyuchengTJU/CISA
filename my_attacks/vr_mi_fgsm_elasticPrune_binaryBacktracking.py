...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
        ...
        for _ in range(iterations):
            ...
            #在查询之前，先判断一下是否会超过当前最优的对抗样本的距离(best dist)，如果会，则不必再查询
            temp_dist = (np.sum((x/255.0 - original/255.0) ** 2))**0.5
            if self.best_adv_img is not None:
                if temp_dist>self.best_dist:
                    return success

            logits, is_adversarial = a.predictions(x)
            self.shit+=1

            if self.best_adv_img is None:
                if is_adversarial:
                    self.best_adv_img = x
                    self.best_dist = temp_dist
            ...
            if is_adversarial:
                ...
                # print("dist", (np.sum((x/255.0 - original/255.0) ** 2))**0.5)
                ...
                for binary_counter in range(10):
                    ...
                    if temp_is_adversarial: #中奖了
                        right = middle
                        self.best_adv_img = middle
                        self.best_dist = (np.sum((middle/255.0 - original/255.0) ** 2))**0.5
                    else:
                        left = middle
