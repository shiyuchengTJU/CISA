...
class CarliniWagnerL2Attack(Attack):
    ...
    def __call__(self, input_or_adv, label=None, unpack=True,
        ...
        for binary_search_step in range(binary_search_steps):
            ...
            for iteration in range(max_iterations):
                ...
                # #FIXME
                # print("x max min", np.max(x), np.min(x), np.mean(x))
    ...
    def loss_function(cls, const, a, x, logits, reconstructed_original,
        ...
        # #FIXME
        # print("total_loss, total_loss_grad", total_loss, total_loss_grad)
...
class AdamOptimizer:
    ...
    def __call__(self, gradient, learning_rate,
        ...
        #FIXME
        # print("v_hat, np.sqrt(v_hat), epsilon", v_hat, np.sqrt(v_hat), epsilon)
        # print("self.v, bias_correction_2", self.v, bias_correction_2)
