...
json_save_path = "/home/syc/adversarial_machine_learning/nips18-avc-attack-template__/temp_json/"


def save_json(ori_img, ori_labels, adv_labels, imgName, doc, step_num):
    # print(type(ori_img), ori_img)
    # print(type(ori_labels), ori_labels)
    # print(type(adv_labels), adv_labels)
    ...
    temp_dict = {}
    temp_dict["ori_img"] = ori_img
    temp_dict["ori_labels"] = ori_labels
    temp_dict["adv_labels"] = adv_labels
    temp_dict["imgName"] = imgName
    temp_dict["doc"] = doc
    temp_dict["step_num"] = step_num

    outFile=codecs.open(json_save_path+'try.json', 'w', encoding='utf-8')
    json.dump(temp_dict, outFile, separators=(',', ':'), sort_keys=True, indent=4) ### this saves the array in .json format
    outFile.close()
...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
        ...
        s = max_ - min_
        doc = []
        step_num = 1  #first step is also included

        original = a.original_image.copy()
        doc.append(original.tolist())
        ...
        for _ in range(iterations):
            ...
            if is_adversarial:
                if return_early:
                    save_json(original.tolist(), a.original_class, np.argmax(logits), "test", doc, step_num)
                    return True
                else:
                    save_json(original.tolist(), a.original_class, np.argmax(logits), "test", doc, step_num)
                    success = True
