...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run_binary_search(self, a, epsilon, stepsize, iterations,
        ...
        for i in range(k):
            ...
            if try_epsilon(epsilon):
                ...
                if log_or_not:
                    logging.info('successful for eps = {}'.format(epsilon))
                # print("factor * epsilon, ", factor * epsilon)
            else:
                bad = epsilon
                if log_or_not:
                    logging.info('not successful for eps = {}'.format(epsilon))
            ...
            #FIXME   查看最终步长
            if i == k-1:  #最后一步
                print("final_step_size, ", factor * epsilon)
    ...
    def _run_one(self, a, epsilon, stepsize, iterations,
        ...
        for _ in range(iterations):
            ...
            # print("min and max of gradient", np.min(gradient), np.max(gradient))
            # print("min and max of x", np.min(x), np.max(x))
            # print("legal min and max", min_, max_)
        ...
        # #FIXME
        # print("success, stepsize", success, stepsize)
        return success
