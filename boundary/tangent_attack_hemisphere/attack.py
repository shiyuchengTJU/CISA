...
sys.path.append("./boundary/tangent_attack_hemisphere")
...
from boundary.tangent_attack_hemisphere.scipy_find_tangent_point import solve_tangent_point
from boundary.tangent_attack_hemisphere.tangent_point_analytical_solution import TangentFinder

class TangentAttack(object):
    def __init__(self, model,  clip_min, clip_max, height, width, channels, norm, epsilon, iterations=40, gamma=1.0,
                 stepsize_search='geometric_progression',
                 max_num_evals=1e3, init_num_evals=10, maximum_queries=10000, batch_size=1,
    ...
    def project(self, original_image, perturbed_images, alphas):
        ...
        alphas = alphas.view(*alphas_shape).cuda()
        if self.norm == 'l2':
    ...
    def geometric_progression_for_tangent_point(self, x_original, x_boundary, normal_vector, true_labels, target_labels,
        ...
        while True:
            ...
            #     continue
            # else:
            tangent_finder = TangentFinder(x_original.reshape(-1), x_boundary.reshape(-1), radius, normal_vector.reshape(-1),
                                           norm="l2")
            ...
            tangent_point = tangent_point.view_as(x_original).type(x_original.dtype).unsqueeze(0)
...
def TangentAttack_process(image_ori, init_adv, model, label_ori, max_query_num, args):
    ...
    # model.cuda()
    # model.eval()
    attacker = TangentAttack(model, 0, 255.0, temp_height, temp_width, temp_channel,
