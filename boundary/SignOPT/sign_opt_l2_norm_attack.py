...
def temp_forward_one(model, x):
    if x.ndim == 4:
        x = x[0]

    temp_logits =  model.forward_one(np.round(x.cpu().numpy()))
    temp_output = torch.from_numpy(temp_logits).unsqueeze(0).cuda()

    return temp_output


class SignOptL2Norm(object):
    def __init__(self, model, epsilon, targeted, batch_size=1, k=200, alpha=0.2, beta=0.001, iterations=1000,
    ...
    def fine_grained_binary_search_local(self,  x0, y0, theta, initial_lbd=1.0, tol=1e-5):
        ...
        if temp_forward_one(self.model, x0 + lbd * theta).max(1)[1].item() == y0:
            lbd_lo = lbd
            lbd_hi = lbd * 1.01
            nquery += 1
            # while self.model(x0 + lbd_hi * theta).max(1)[1].item() == y0:
            while temp_forward_one(self.model, x0 + lbd_hi * theta).max(1)[1].item() == y0:
                lbd_hi = lbd_hi * 1.01
                nquery += 1
            ...
            while temp_forward_one(self.model, x0 + lbd_lo * theta).max(1)[1].item() != y0:
                lbd_lo = lbd_lo * 0.99
        ...
        while (lbd_hi - lbd_lo) > tol:
            ...
            if temp_forward_one(self.model, x0 + lbd_mid * theta).max(1)[1].item() != y0:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
            if tot_count>200:
    ...
    def fine_grained_binary_search_local_targeted(self, x0, t, theta, initial_lbd=1.0, tol=1e-5):
        ...
        if temp_forward_one(self.model, x0 + lbd * theta).max(1)[1].item() != t:
            lbd_lo = lbd
            lbd_hi = lbd * 1.01
            nquery += 1
            # while self.model(x0 + lbd_hi * theta).max(1)[1].item() != t:
            while temp_forward_one(self.model, x0 + lbd_hi * theta).max(1)[1].item() != t:
                lbd_hi = lbd_hi * 1.01
                nquery += 1
                if lbd_hi > 100:
                    return float('inf'), nquery
            ...
            while temp_forward_one(self.model, x0 + lbd_lo * theta).max(1)[1].item() == t:
                lbd_lo = lbd_lo * 0.99
                nquery += 1
        tot_count = 0
        while (lbd_hi - lbd_lo) > tol:
            ...
            if temp_forward_one(self.model, x0 + lbd_mid * theta).max(1)[1].item() == t:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
            if tot_count>200:
    ...
    def fine_grained_binary_search(self,  x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            ...
            if temp_forward_one(self.model, x0 + current_best * theta).max(1)[1].item() == y0:
                return float('inf'), nquery
            lbd = current_best
        else:
            lbd = initial_lbd
    ...
    def fine_grained_binary_search_targeted(self, x0, t, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            ...
            if temp_forward_one(self.model, x0 + current_best * theta).max(1)[1].item() != t:
                return float('inf'), nquery
            lbd = current_best
        else:
            lbd = initial_lbd
    ...
    def sign_grad_svm(self, images, true_label, theta, initial_lbd, h=0.001, K=100, target_label=None):
        ...
        if target_label is not None:
            ...
            # predict_labels = self.model(images_batch).max(1)[1]
            predict_labels = temp_forward_one(self.model, images_batch).max(1)[1]
            sign[predict_labels!=true_labels] = -1

        X = torch.transpose(u_batch.reshape(K, dim) * sign.reshape(K,1),0,1)  # convert from X[:, iii] = sign * u.reshape(dim)
        ...
        sign_grad = torch.matmul(X, alpha).reshape_as(theta)
    ...
    def sign_grad_v1(self, images, true_label, theta, initial_lbd, h=0.001, target_label=None):
        ...
        if target_label is not None:
            ...
            predict_labels = temp_forward_one(self.model, images_batch).max(1)[1]
            sign[predict_labels!=true_labels] = -1
        sign_grad = torch.sum(u_batch * sign.reshape(K,1,1,1),dim=0,keepdim=True)

        sign_grad = sign_grad / K

        return sign_grad, queries

    def untargeted_attack(self, images, true_labels, init_noise, best_distance):
        ...
        batch_image_positions = np.arange(0 * self.batch_size,
                                          min((0 + 1) * self.batch_size, self.total_images)).tolist()
        ...
        if temp_forward_one(self.model, images + theta).max(1)[1].item() == true_label:
            #不用跑了
            return images + theta, query
        ...
        #     target = temp_forward_one(self.model, images + gg * xg).max(1)[1].item()
    ...
    def get_image_of_target_class(self,dataset_name, target_labels, target_model):
        ...
        for label in target_labels:  # length of target_labels is 1
            ...
            with torch.no_grad():
                ...
                logits = temp_forward_one(target_model, image.cuda())
            while logits.max(1)[1].item() != label.item():
                ...
                with torch.no_grad():
                    ...
                    logits = temp_forward_one(target_model, image)
    ...
    def count_stop_query_and_distortion(self, images, perturbed, query, success_stop_queries,
        ...
        dist = torch.norm((perturbed - images).reshape(images.size(0), -1), p=2, dim=1)
