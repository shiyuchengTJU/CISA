...
from boundary.SignOPT.attack import SignOPT_process

from boundary.tangent_attack_hemisphere.attack import TangentAttack_process
from boundary.tangent_attack_semiellipsoid.attack import EllipsoidTangentAttack_process
from boundary.SignOPT.attack import SignOPT_process
...
# Methods for Transfer Attack Module
...
# Methods for Noise Compression Module
...
def sign_opt_refinement(model, image, label, max_query_num, args, init_adv, best_distance):
    adversarial_ori_sign_opt, used_query_num = SignOPT_process(model, image, label, max_query_num, args, init_adv-image, best_distance)

    return adversarial_ori_sign_opt
...
def main(arvg):
    ...
    # Substitute (backward) and Target (forward) Model Choice
    ...
    for (file_name, image, label) in read_images():
        ...
        # Generate Intermediate Adversarial Example with Different Parameter Adjustment Module and Transfer Attack Module
        ...
        # Our Gaussian Stepsize Adjustment + Iterative Trajectory Diversification
        ...
        # Compress Noise with Different Transfer Attack Module and State Transition Module

        # Transfer Attack Only. No Noise Compression
        ...
        # Boundary Attack as Noise Compression
        ...
        # Biased Boundary Attack as Noise Compression
        if check_1:
            #######################
            temp_adv_list[3] = perlin_refinement(image, adversarial_ori_1, model, label, int(return_1), source_step=args.source_step, spherical_step=args.spherical_step, pixels=args.big_size)
            #######################
        ...
        # Sign-Opt Attack as Noise Compression
        if check_1:
            #######################
            temp_adv_list[4] = sign_opt_refinement(model, image, label, args.all_access, args, adversarial_ori_1, return_1)
            #######################
        ...
        # QEBA as Noise Compression
        if check_1:
            #######################
            temp_adv_list[5] = qeba_refinement(image, adversarial_ori_1, model, label, int(return_1))
            #######################
        ...
        #   Tangent Attack as Noise Compression
        if check_1:
            #######################
            temp_adv_list[6] = TangentAttack_process(image, adversarial_ori_1, forward_model, label, int(return_1), args)
            #######################
        ...
        # Evolutionary Attack as Noise Compression
        if check_1:   #允许攻击
            #######################
            temp_adv_list[7] = boundary_refinement(image, adversarial_ori_1, model, label, int(return_1), 2, source_step=args.source_step, spherical_step=args.spherical_step, big_size=args.big_size, center_size=args.center_size)
            #######################
            aux_dist[7].append(l2_distance(temp_adv_list[7], image))
        else:
            aux_dist[7].append(return_1)

        # HSJA as Noise Compression
        if check_1:   
            #######################
            temp_adv_list[8], evolutionary_doc_5 = boundary_refinement(image, adversarial_ori_1, model, label, int(return_1), 39, source_step=args.source_step, spherical_step=args.spherical_step, rate=args.rate, big_size=args.big_size, center_size=args.center_size)
            #######################
            aux_dist[8].append(l2_distance(temp_adv_list[8], image))
        else:
            aux_dist[8].append(return_1)

        # Our Customized Sampling + Transition Function Relaxation as Noise Compression
        if check_1:   #允许攻击
            #######################
            temp_adv_list[9] = boundary_refinement(image, adversarial_ori_1, model, label, int(return_1), 39, source_step=args.source_step, spherical_step=args.spherical_step, rate=args.rate, big_size=args.big_size, center_size=args.center_size)
            #######################
            aux_dist[9].append(l2_distance(temp_adv_list[9], image))
        else:
            aux_dist[9].append(return_1)

        # Print Noise Magnitude for each Attack
